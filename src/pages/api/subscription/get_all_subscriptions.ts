...
export default async function getAllSubscriptions(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== "POST") {
    ...
    try {
        ...
        const subscriptions_ref = firestore.collection(`projects/${projectUID}/subscriptions`);
        const snapshot = await subscriptions_ref.get();
